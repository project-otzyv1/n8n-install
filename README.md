...
👉 Чтобы включить HTTP Basic Auth, добавь флаг `--enable-basic-auth`:

```bash
bash <(curl -s https://raw.githubusercontent.com/kalininlive/n8n-beget-install/main/install.sh) --enable-basic-auth
```
...
* (если использован `--enable-basic-auth`) имя пользователя и пароль для входа в n8n
* Ключ шифрования (или сгенерируется)
* Токен Telegram-бота
* Telegram ID (куда слать уведомления)

После завершения установки открой `https://<твой_домен>` и, если была включена basic auth, войди, используя указанные имя пользователя и пароль.
